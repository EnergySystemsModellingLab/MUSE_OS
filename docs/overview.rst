...
Next, the MUSE sector modules run. For this, multiple iterations are run to calculate marginal cost and supply (from the sectors), as well as the corresponding commodity price and demand. Once the MCA has converged, after carrying out these iterations, the converged price is carried forward to generate the updated forward price trajectory. At the next time period, investments can be made based on the updated forward price, the corresponding demand trajectory, and the known technology's techno-economic data. This whole process repeats itself until the specified number of milestone years have run.
