...
def _inner_split(
    ...
    try:
        # calculates the total demand assigned in the previous step with the "method" function across agents and assets. The previous version of this calculation only calculated the sum of assets present in all "shares.values()" (which contain agents). 
    ...
    # calculates the demand divided by the number of assets times the number of agents if the demand is bigger than zero and the total demand assigned with the "method" function is zero. In the previous version, the demand was divided by the number of agents times the number of assets present in all "shares.values()" (which represent agents) and thus could become inf if all agents owned different assets.
    unassigned = (
